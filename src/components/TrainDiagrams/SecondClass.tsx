...
const SecondClass = () => {
  ...
  const secondClassWagons: Wagon[] = (renderClassType.find((item) => item.name === "second")?.wagons ?? []) as Wagon[];

  if (!secondClassWagons) {
    return null;
  }

  const processedData = secondClassSeats(secondClassWagons);
